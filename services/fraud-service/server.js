...
app.get('/fraud/health', (req, res) => {
  res.status(200).json({ status: 'healthy', service: 'fraud-service' });
});

// PROTECTED ROUTES
app.get('/api/fraud', securityAuthenticator, async (req, res) => {
  try {
    res.json({ success: true, service: 'fraud-analysis', status: 'Clear', checks: [] });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

app.listen(PORT, () => console.log(`Fraud microservice live on port ${PORT}`));